...
All UI in this project is built on **`@galacius/design-system`** (currently `^1.6.3`). It is the only component library — do not add another UI kit (Radix, shadcn/ui, MUI, etc.) or hand-roll a primitive (button, dialog, dropdown, etc.) that the design system already provides. Reach for raw Tailwind utility classes only for one-off layout (flex/grid, spacing, positioning) that isn't a reusable primitive.

Repo: <https://github.com/galacius/galacius> (`design-system` directory).
...
| Entrypoint                           | Contents                                                                                                                                                                                                                                                                                            |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@galacius/design-system`            | Everything (barrel of all entrypoints below)                                                                                                                                                                                                                                                        |
| `@galacius/design-system/atoms`      | Primitives: `Button`, `Input`, `Badge`, `Checkbox`, `Select`, `Dialog`, `Sheet`, `Drawer`, `DropdownMenu`, `ContextMenu`, `Tabs`, `Table`, `Tooltip`, `Switch`, `Slider`, `Separator`, `ScrollArea`, `Textarea`, `Toaster`, lucide-react icons                                                      |
| `@galacius/design-system/components` | Higher-level composites: `EmptyState`, `ConfirmationModal`, `FormModal`, `LoadingSpinner`, `Markdown`, `Divider`, `ButtonGroup`, `TruncatedText`, `DonutChart`, resource-oriented components (`ResourceCell`, `ResourceDetailDrawer`, etc. — app-specific, unlikely to be needed on a landing page) |
| `@galacius/design-system/hooks`      | `useCopyToClipboard`, `useMediaQuery`, `useBreakpoint` (+ `BREAKPOINTS` constant)                                                                                                                                                                                                                   |
| `@galacius/design-system/utils`      | `cn`, `clamp`, `formatRelativeTime`, `formatTs`                                                                                                                                                                                                                                                     |
| `@galacius/design-system/types`      | Shared TS types (`NavItem`, `NavGroup`, etc.)                                                                                                                                                                                                                                                       |
| `@galacius/design-system/libs`       | `FullTextSearchInput`, `useFullTextSearch`                                                                                                                                                                                                                                                          |

Prefer importing from `./atoms` or `./components` directly over the root barrel for clarity on where a component lives.

## Class name conventions

- Use `cn` from `@galacius/design-system/utils` (not raw template strings or `clsx` directly) whenever merging conditional or override class names — it wraps `clsx` + `tailwind-merge` so conflicting utilities resolve correctly.
...
- When component _state_ also needs to react to a breakpoint crossing (e.g. auto-closing a mobile-only `Sheet` when the viewport grows into the desktop layout), use `useBreakpoint` from `@galacius/design-system/hooks` instead of a hand-rolled `window.matchMedia` listener — it matches the same `sm`/`md`/`lg`/`xl`/`2xl` scale as the Tailwind utilities via its `BREAKPOINTS` map. See `src/layouts/header/HeaderMobileView.tsx` for the pattern: `const isDesktop = useBreakpoint("md")`, then close state in a `useEffect` keyed on `isDesktop`.

## Icons & feedback

- Icons: use the `lucide-react` icons re-exported from the atoms entrypoint (e.g. `import { RocketIcon } from "@galacius/design-system"`) rather than adding a new icon package.
